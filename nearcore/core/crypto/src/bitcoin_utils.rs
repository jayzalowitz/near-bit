...
pub fn derive_bitcoin_address_from_pubkey(pubkey: &Secp256K1PublicKey) -> String {
    ...
    uncompressed[0] = 0x04;
    uncompressed[1..65].copy_from_slice(pubkey_bytes);

    // Compress to 33 bytes (0x02 or 0x03 prefix + 32 bytes X coordinate)
    let prefix = if pubkey_bytes[63] & 1 == 0 { 0x02u8 } else { 0x03u8 };
    let mut compressed = [0u8; 33];
    compressed[0] = prefix;
    compressed[1..33].copy_from_slice(&pubkey_bytes[0..32]);
    ...
    // Base58Check encode using canonical Bitcoin casing.
...
pub fn derive_all_bitcoin_addresses(pubkey: &Secp256K1PublicKey) -> Vec<String> {
    let p2pkh = derive_bitcoin_address_from_pubkey(pubkey);
    let p2wpkh = derive_bitcoin_p2wpkh_from_pubkey(pubkey);

    // Keep legacy lowercased Base58 as a compatibility alias for older
    // snapshots that normalized addresses.
    let p2pkh_legacy = p2pkh.to_lowercase();
    if p2pkh_legacy != p2pkh { vec![p2pkh, p2wpkh, p2pkh_legacy] } else { vec![p2pkh, p2wpkh] }
}
...
mod tests {
    ...
    fn test_bitcoin_address_derivation() {
        ...
        let pubkey_bytes = [
            0x79, 0xbe, 0x66, 0x7e, 0xf9, 0xdc, 0xbb, 0xac, 0x55, 0xa0, 0x69, 0x24, 0xef, 0xb5,
            0x22, 0xfe, 0x9a, 0x05, 0x1f, 0xb7, 0x01, 0x6d, 0x83, 0x6d, 0x35, 0x26, 0x78, 0xd9,
            0x47, 0x81, 0xd5, 0xd9, 0x7c, 0xe4, 0x26, 0x9b, 0x28, 0x92, 0xd0, 0xb8, 0x99, 0xb1,
            0x0d, 0x6b, 0x97, 0x8b, 0x5b, 0x1d, 0x17, 0xd0, 0x05, 0x4b, 0x1f, 0x0f, 0x4d, 0x0d,
            0x0b, 0xb6, 0x3a, 0xa0, 0xbb, 0x1c, 0x8e, 0x9a,
        ];
    ...
    fn test_derive_all_addresses() {
        ...
        assert_eq!(addresses[2], addresses[0].to_lowercase()); // Legacy alias
    }
}
